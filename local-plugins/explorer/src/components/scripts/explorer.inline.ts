...
function renderTree(
  ...
  if (node.isFolder) {
    ...
    // Check saved state for collapsed status, falling back to the configured default open depth
    const isCollapsed =
      savedState[node.slug] !== undefined ? savedState[node.slug] : depth > defaultOpenDepth;
...
async function handleNavOrRender(e) {
  ...
  try {
    ...
    for (const explorer of allExplorers) {
      ...
      const depthRaw = explorer.dataset.collapsedDepth;
      const defaultOpenDepth = depthRaw === "Infinity" ? Infinity : parseInt(depthRaw, 10) || 0;
